...
module "network-eastus" {
  ...
  subnet_names        = ["aks-system", "north001", "north002"]
...
module "network-westeurope" {
  ...
  subnet_prefixes     = ["10.53.0.0/24", "10.53.1.0/24", "10.53.2.0/24"]
  subnet_names        = ["aks-system", "west001", "west002"]
...
resource "azurerm_role_assignment" "aks-eastus" {
  principal_id         = module.aks-eastus.system_assigned_identity[0].principal_id
...
resource "azurerm_role_assignment" "aks-westeurope" {
  principal_id         = module.aks-westeurope.system_assigned_identity[0].principal_id
  role_definition_name = "Network Contributor"
